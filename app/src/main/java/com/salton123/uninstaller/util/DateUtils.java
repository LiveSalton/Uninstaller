...
import com.salton123.uninstaller.XApp;
...
public class DateUtils {
    ...
    public static String timeFormatNearby(Date date) {
        ...
        if (nowCalender.get(Calendar.YEAR) != sendDateYear) {
            sb.append(sendDateYear).append(XApp.getInstance().getString(R.string.year_prefix));
            sb.append(sendDateMonth).append(XApp.getInstance().getString(R.string.month_prefix));
        ...
        // 今天
        if (isToday(date)) {
            sb.append(XApp.getInstance().getString(R.string.today));
            return sb.toString();
        }

        // 昨天
        if (isYesterday(date)) {
            sb.append(XApp.getInstance().getString(R.string.yesterday));
            return sb.toString();
        }
        ...
        if (sendCalender.get(Calendar.WEEK_OF_YEAR) == nowCalender.get(Calendar.WEEK_OF_YEAR)) {
            switch (sendCalender.get(Calendar.DAY_OF_WEEK)) {
                case Calendar.MONDAY:
                    sb.append(XApp.getInstance().getString(R.string.monday));
                    break;
                case Calendar.TUESDAY:
                    sb.append(XApp.getInstance().getString(R.string.tuesday));
                    break;
                case Calendar.WEDNESDAY:
                    sb.append(XApp.getInstance().getString(R.string.wednesday));
                    break;
                case Calendar.THURSDAY:
                    sb.append(XApp.getInstance().getString(R.string.thursday));
                    break;
                case Calendar.FRIDAY:
                    sb.append(XApp.getInstance().getString(R.string.friday));
                    break;
                case Calendar.SATURDAY:
                    sb.append(XApp.getInstance().getString(R.string.saturday));
                    break;
                default:
                    sb.append(XApp.getInstance().getString(R.string.sunday));
            }
            return sb.toString();
        }
